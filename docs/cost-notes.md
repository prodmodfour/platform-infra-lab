...
Other implemented VPC components such as subnets, route tables, an internet gateway, and security groups are part of the network/security model. Security groups do not have standalone hourly cost, but they enable future resources that do: ALB traffic, ECS/Fargate tasks, RDS PostgreSQL, optional Redis/ElastiCache, NAT egress, and CloudWatch logs/metrics.
