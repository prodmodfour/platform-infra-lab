...
variable "load_balancer_internal" {
  description = "Whether the Application Load Balancer is internal. Dev defaults to false to model the public edge."
  type        = bool
  default     = false
}

variable "load_balancer_deletion_protection_enabled" {
  description = "Whether ALB deletion protection is enabled. Dev keeps this false for easier cleanup of user-owned experiments."
  type        = bool
  default     = false
}
...
variable "load_balancer_access_logs_enabled" {
  description = "Whether ALB access logs are enabled. Dev keeps this false because no real log bucket is committed."
  type        = bool
  default     = false
}
...
variable "secrets_manager_recovery_window_in_days" {
  description = "Secrets Manager recovery window for metadata-only ECS secret containers in dev. No secret values are managed by Terraform."
  type        = number
  default     = 7
...
variable "service_example_profiles" {
  ...
  default = {
    ...
    job-runner-platform = {
      ...
      cache_requirement            = "Can use private Redis/Valkey when enabled for queue coordination or worker leases; dev keeps cache resources disabled by default."
      metrics_expectations = [
...
variable "alb_ingress_cidrs" {
  description = "IPv4 CIDR blocks allowed to reach the public ALB. Dev uses public internet in the example so the ALB is the only public ingress boundary."
  type        = list(string)
  default     = ["0.0.0.0/0"]
...
variable "alb_ingress_ports" {
  description = "TCP ports exposed on the public ALB security group. Dev defaults to HTTP only for the placeholder edge."
  type        = list(number)
  default     = [80]
...
variable "rds_database_name" {
  description = "Initial PostgreSQL database name for the dev RDS instance. This is not a secret."
  type        = string
  default     = "appdb"
...
variable "rds_engine_version" {
  description = "PostgreSQL engine version for the dev RDS example. Review regional support before manual provisioning."
  type        = string
  default     = "16.3"
...
variable "rds_instance_class" {
  description = "RDS instance class for the dev PostgreSQL example."
  type        = string
...
variable "rds_allocated_storage_gib" {
  description = "Initial allocated storage in GiB for the dev PostgreSQL instance."
  type        = number
  default     = 20
...
variable "rds_multi_az" {
  description = "Whether the PostgreSQL instance uses Multi-AZ. Dev disables this by default to stay cost-aware."
  type        = bool
  default     = false
}
...
variable "rds_deletion_protection_enabled" {
  description = "Whether PostgreSQL deletion protection is enabled. Dev disables this for disposable lab experiments."
  type        = bool
  default     = false
}
