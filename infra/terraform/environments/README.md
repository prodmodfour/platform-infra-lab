...
`dev` should demonstrate the platform pattern with lower-cost defaults where practical. Examples include smaller desired counts, shorter log retention, optional NAT gateway usage, disabled cache resources unless explicitly enabled, smaller database/cache sizing, and relaxed deletion protection where safe for a lab.

`prod` should demonstrate production intent rather than guarantee production readiness. Examples include higher availability settings, stronger log retention, deletion protection for stateful services, private Multi-AZ PostgreSQL, an optional private Redis cache with replicas/Multi-AZ intent, tighter scaling thresholds, and clear notes about remaining hardening work.
