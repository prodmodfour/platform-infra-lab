...
This root module is the public-safe `prod` environment for `platform-infra-lab`. It mirrors the dev structure while using production-intent defaults and examples for review, including the shared network, security-groups, IAM, and ECS service modules.
...
Future tickets add the load balancer, RDS PostgreSQL, Redis resources, and observability.
